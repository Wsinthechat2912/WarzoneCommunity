...
const ExperiencedGamerScreen = () => {
  ...
  useEffect(() => {
    const handleServersUpdate = (userServers) => {
      setServers(userServers);
    };

    ServerService.fetchUserServers("experienced", handleServersUpdate);
  }, []);
  ...
  const ServerItem = ({ server }) => (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("ExperiencedServerChat", { serverId: server.id })
      }
      activeOpacity={0.7}
    >
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Experienced Gamer's Hub</Text>
      </View>
      <FlatList
        data={servers}
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("ExperiencedManagingServers")}
          activeOpacity={0.8}
        >
